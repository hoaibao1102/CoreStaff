...
function build(userOrUsers: UserRow | UserRow[] | null) {
	...
	// login's $or(emailN | regex employeeCode) and forgotPassword's
	// { emailN, status }. Unknown keys are ignored (fake, not Mongo).
	const matches = (u: UserRow, q: Record<string, unknown>): boolean => {
		if (Array.isArray(q.$or)) return (q.$or as Record<string, unknown>[]).some((sub) => matches(u, sub));
		if (q.emailN !== undefined && u.emailN !== q.emailN) return false;
		if (q.employeeCode !== undefined) {
			const rx = q.employeeCode as { $regex: string; $options?: string };
			const code = u.employeeCode ?? '';
			const re = new RegExp(rx.$regex, rx.$options);
	...
	const userModel = {
		...
		find(q: unknown) {
			// `find` for the email-fan-out in forgotPassword: match all emailN holders.
			const query = q as Record<string, unknown>;
			return {
				exec: async () =>
					users.filter((u) => u.emailN === query.emailN && (query.status === undefined || u.status === query.status)),
			};
		},
		findById(id: string) {
...
describe('AuthService forgot/reset password (TASK-016, FR-AUTH-05)', () => {
	...
	it('forgotPassword fans out to every ACTIVE tenant sharing the address; tokens stay user-bound', async () => {
		...
		const a = makeUser({ _id: 'uA', email: shared, emailN: shared, employeeCode: 'A-1', organizationId: 'orgA', passwordHash: await hashPassword(PASSWORD) });
		const b = makeUser({ _id: 'uB', email: shared, emailN: shared, employeeCode: 'B-1', organizationId: 'orgB', passwordHash: await hashPassword(PASSWORD) });
