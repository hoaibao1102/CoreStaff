...
import {
  ...
  OrganizationAllowanceSchema, OvertimePayPolicySchema, SalaryProfileSchema,
...
async function main() {
  ...
  const Overtime = db.model('OvertimePayPolicy', OvertimePayPolicySchema);
  let errors = 0;
  const orgs = await Org.find({}).select('_id code').sort({ code: 1 }).lean();
  for (const org of orgs) {
    const [profiles, salaries, labor, allowances, bonuses, kpis, overtimes] = await Promise.all([
      ...
      Kpi.find({ organizationId: org._id }).lean(), Overtime.find({ organizationId: org._id }).lean(),
    ]);
    ...
    console.log(`${org.code}: profiles=${profiles.length} salaries=${salaries.length} labor=${labor.length} overtimePolicies=${overtimes.length} allowances=${allowances.length} bonusPolicies=${bonuses.length} kpis=${kpis.length} (confirmed=${confirmed}, draft=${draft}, probation=${probation})`);
  }
