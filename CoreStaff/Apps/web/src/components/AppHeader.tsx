...
export function AppHeader({ roleLabel, user, onLogout }: AppHeaderProps) {
  ...
  const links = [
    ...
    { href: '/hr/employees', label: 'Nhân viên', icon: Users, visible: canViewEmployees(user) },
    { href: '/app/profile', label: 'Hồ sơ của tôi', icon: UserRound, visible: canViewProfile(user) },
  ];
  return (
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <AppLink href="/" aria-label="CoreStaff — Tổng quan" className="shrink-0 rounded-lg focus-visible:outline-2 focus-visible:outline-blue-600"><CoreStaffLogo /></AppLink>
      ...
      <nav aria-label="Điều hướng chính" className="mx-auto flex max-w-7xl gap-2 overflow-x-auto px-4 sm:px-6 lg:px-8">
        {links.filter(link => link.visible).map(({ href, label, icon: Icon }) => {
          const active = path === href || (href === '/' && path === '/hr/dashboard');
