...
export type RequestType='ATTENDANCE'|'OVERTIME';export type RequestStatus='PENDING'|'APPROVED'|'REJECTED'|'CLARIFICATION_REQUESTED';
export interface ManagerRequest{_id:string;employeeId:string;employeeUserId:string;departmentId:string;type:RequestType;workDate:string;reason:string;requestedStart?:string;requestedEnd?:string;approvedStart?:string;approvedEnd?:string;status:RequestStatus;reviewComment?:string;version:number;createdAt?:string}
export interface ManagerEmployee{id:string;userId:string;employeeCode:string;fullName?:string|null;departmentId?:string|null;positionId?:string|null;employmentStatus:string}
